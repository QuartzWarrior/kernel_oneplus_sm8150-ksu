...
/* Extended constants for SukiSU-Ultra builtin branch */
...
/* Compat helpers expected by SukiSU-Ultra builtin branch */
#include <linux/string.h>
#include <linux/sched.h>

/* In v1.5.5, per-process umount state is tracked via susfs_task_state */
#define TIF_PROC_UMOUNTED_COMPAT BIT(24) /* TASK_STRUCT_NON_ROOT_USER_APP_PROC */
...
{
	size_t sl = strlen(str), pl = strlen(suffix);
	if (pl > sl) return false;
	return !strcmp(str + sl - pl, suffix);
}
