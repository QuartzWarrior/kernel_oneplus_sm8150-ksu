				 ...
				 int *__never_use_flags)
{
	...
	if (!ksu_is_allow_uid_for_current(current_uid().val) && !is_uid_manager(current_uid().val))
		return 0;
	...
	/* Redirect to ksud if it exists, otherwise fall back to sh */
	{
		struct file *f = ksu_filp_open_compat(ksud_path, O_RDONLY, 0);
		if (IS_ERR(f)) {
			static const char sh_path[] = "/system/bin/sh";
			pr_warn("ksud not found, falling back to sh\n");
