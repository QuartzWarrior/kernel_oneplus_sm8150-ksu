...
/* Legacy prctl detection + fd bootstrap for non-GKI built-in builds.
 * The manager falls back to prctl(0xDEADBEEF, 2, ...) when the
 * [ksu_driver] fd is not yet in its process.  We handle it here to
 * crown the caller as manager (first call only) and install the fd,
 * breaking the circular dependency between detection and fd install. */
